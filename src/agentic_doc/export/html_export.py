...
def write_html(
    ...
    """Write interactive HTML with image + SVG overlay and transcription side-by-side.

    Bbox alignment requires one thing: the SVG must occupy the exact same box
    as the rendered ``<img>``. We get that by wrapping image and SVG together
    in an ``.img-wrap`` element whose size is dictated by the image, and by
    setting ``align-items: flex-start`` on the row so the image side doesn't
    get stretched to match the (taller) transcription side.

    The previous version put ``<img>`` and ``<svg>`` directly inside a flex
    item and relied on ``align-items: stretch`` (the default) plus
    ``height: auto`` on the image. When the transcription column was taller
    than the image, the column was stretched, the image stayed at its real
    height, the SVG (sized to the column) ended up with a different aspect
    ratio than its viewBox, and ``preserveAspectRatio`` then re-centered the
    overlay inside the bigger box — that's the off-by-many-pixels misalignment.
    """
    ...
    # SVG overlay shapes (region rects + line polygons / rects).
    svg_parts: List[str] = []
    for region in regions:
        ...
        svg_parts.append(
            f'<rect class="region-g" data-rid="{rid}" '
        ...
        for line in region.get("lines", []):
            ...
            if poly:
                ...
                svg_parts.append(
                    f'<polygon class="line-poly" data-lid="{lid}" data-rid="{rid}" '
                    f'points="{pts}" fill="none" stroke="#00ff00" '
                ...
                svg_parts.append(
                    f'<rect class="line-poly" data-lid="{lid}" data-rid="{rid}" '
                    f'x="{lx}" y="{ly}" width="{lw}" height="{lh}" '
                    f'fill="none" stroke="#00ff00" stroke-width="1" stroke-dasharray="4"/>'
                )
    svg_content = "\n".join(svg_parts)
    ...
    for region in regions:
        ...
        if rtype == "TableRegion" and region.get("text", "").strip().startswith("|"):
            ...
            for line in region.get("lines", []):
                ...
                text = line.get("text", "")
                trans_parts.append(
                    f'<span class="line-span" data-lid="{lid}" data-rid="{rid}">'
...
.img-wrap {{ position: relative; line-height: 0; display: block; }}
.img-wrap img {{ display: block; width: 100%; height: auto; max-width: 100%; user-select: none; }}
.img-wrap svg {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; overflow: visible; }}
.img-wrap svg rect, .img-wrap svg polygon {{ pointer-events: auto; cursor: pointer; }}
...
<div class="layout">
  <div class="img-panel">
    <div class="img-wrap">
      <img id="docImg" src="data:image/png;base64,{b64}" alt="Document"/>
      <svg id="overlay" viewBox="0 0 {img_w} {img_h}" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
        {svg_content}
      </svg>
    </div>
  </div>
...
(function() {{
  ...
  function clearSel() {{
    document.querySelectorAll('.line-span.sel, .line-poly.sel, .read-region.sel, .region-g.sel')
